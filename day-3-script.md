...
**Block A (2 hours, everyone):** the room picks a submitted use case and we build the skill from it live, then
a ten-minute governance snapshot. **Block B (1.5 hours, by invitation):** a clinic — no curriculum, people
bring what they're stuck on.

**This is the longest day of the three**, and for anyone in both blocks it is three and a half hours in the
same chair. Two things decide whether it lands: **validating the use cases beforehand**, and **protecting the
break**.
...
1. **`TODO` — validate the canvases with Chris and Charles. Not optional.** Read every submitted canvas,
   build a shortlist of **two**, and walk it past Chris and Charles *before* Thursday. The named risk: IT-
   flavoured ideas — **status roll-ups are the specific example** — may not land with executives who *receive*
   reports rather than compile them. A shortlist the room rejects in the first twenty minutes costs you the
   lab, and there is no recovering that inside a 130-minute agenda. Open item on Nimble Gravity; not yet done.
2. **`TODO` — confirm the Gen Re workflow and stage its files.** The live build needs real files in a folder
...
| Time | Slot | Content |
|---|---|---|
| 0:00–0:15 | **Open** | Connect & frame — recap Days 1–2, review submitted canvases |
| 0:15–0:35 | **Teach** | Ideate and agree on an actual Gen Re workflow for the lab |
| 0:35–1:15 | **Lab** | Build a skill from the selected use case — live |
...
| 2 | **Connect & frame** | 15 → 10 min | 5 | The canvases were validated beforehand; present the shortlist, don't re-derive it in the room. |
| 3 | **Teach & demo** | 20 → 15 min | 5 | Only if 1 and 2 weren't enough. Arrive with **two** candidates, not six, and put it to a vote in five minutes. |
...
Wednesday you did a workflow by hand and then wrote it down. Today you bring your own work, we build one of
them in front of everybody, and then ten minutes on the questions your team will ask you — what's connected,
who can turn what on, and where the data goes."*
...
### The canvases · ~8 min
**[DO]** Present the **shortlist of two** — validated with Chris and Charles beforehand — on the projector.
For each: the workflow in one line, the documents it touches, and what "checked and correct" means.

**[SAY] — the honest thank-you and the honest filter:** *"We read all of them. These two are the ones we can
build in forty minutes with files that are actually in the room. That is the only filter — a canvas not on
this list isn't a worse idea, it's a bigger one."*

**[SAY] — name the four criteria as you present, so the choice is transparent:** *"Buildable today means four
things: **documents you could put in a folder right now**; **an output you'd recognise as right**; **nothing
confidential in it**; and **doable in forty minutes.** Everything else is a great second project."*

**[NOTE]** If the shortlist was **not** validated with Chris and Charles, say so and hand the choice to the
room rather than defending a list you can't defend. And expect the status-roll-up problem: an idea that suits
the people who compile reports can land badly with executives who receive them.

---

## Teach & demo · 0:15–0:35 (20 min) — Agree on the workflow

**[SAY] — mode:** *"Hands off for twenty minutes. This bit is a conversation."*

**[DO]** Hold the two candidates against the criteria out loud, one at a time, then **put it to the room and
take a decision.** Do not let this drift — you are buying a commitment, not consensus.

**[SAY] — force the close at 0:28 if needed:** *"We need to pick. Hands for A. Hands for B. That's it —
that's what we're building, and the other one is first on the office-hours list."*

**[DO]** With the choice made, capture the half page on the projector, in the room's words — this is the same
shape as yesterday:
...
**[SAY] — connect it back:** *"That's exactly the half page you wrote for homework. Nothing new. The only
difference is this one is ours and we're going to run it."*
...
## Lab · 0:35–1:15 (40 min) — Build it live
...
**[SAY] — mode:** *"Hands off. Screens down."*

**[DO]** Take shares in this format, ninety seconds each — say the format first and hold them to it:
> **What it got right · what it got wrong · what you'd have to check before anyone else used it.**
...
- **The single biggest risk on Day 3 is an unvalidated shortlist.** Not the live build — the build has a
  backup. A shortlist the room rejects at 0:25 leaves you with no lab and no fallback. Do prep item 1.
...
- **Lines worth keeping verbatim:** *"here's one I built earlier"* · *"a canvas not on this list isn't a worse
  idea, it's a bigger one"* · *"nothing leaves this room as 'we'll see'"* · *"it drafts, you check, you
  sign."*
...
- **Selection criteria** = `pages/training/04-use-cases-by-industry.html#selection`; the canvas =
  `#canvas`; worked examples = `#examples`.
...
- **Backup demo, canvas validation and the break** are all documented as facilitator guidance in
